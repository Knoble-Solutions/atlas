...
function displayCountryInfo(countryData, container) {
    ...
    const flagAlt = countryData.flag.alt ? countryData.flags.alt : `Flag of ${countryData.name.common}`
    ...
    <p class="country-name"><strong>Capital:</strong>${countryData.capital ? countryData.capital.join(',') : 'Not available' }</p>
    ...
    <p><strong>Google:</strong> <a href=${countryData.maps.googleMaps}>LINK</a></p>
    `
...
function displayCountryList (countries, container){
    let listHtml = `<p>Multiple resules returned. Please choose one of the countries below to view: </p><ul>`
